  ...
  try {
    const stockData = await Promise.all(
      tickersArr.map(async (ticker) => {
        ...
        const url = `https://polygon-api-worker.ajbl-segun.workers.dev/?ticker=${ticker}&startDate=${dates.startDate}&endDate=${dates.endDate}`;
        // cloudflare worker URL for polygon proxy- to test
        // https://polygon-api-worker.ajbl-segun.workers.dev/?ticker=MSFT&startDate=2025-11-10&endDate=2025-11-13
...
async function fetchReport(data) {
  ...
  try {
    const url = "https://openai-api-worker.ajbl-segun.workers.dev";
    ...
    if (!response.ok) {
      throw new Error(`There is a worker Error: ${data.error}`);
    }
    const data = await response.json();
    return renderReport(data?.content || "AI returned no response");
  } catch (err) {
    console.error("fetchReport error:", err);
    loadingArea.innerText = `Unable to access AI: ${err.message}`;
  }
}
